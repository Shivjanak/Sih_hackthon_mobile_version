...
# ── MOBILE GEO-PHOTOS & LIVE GPS ENDPOINTS ──────────────────────────────────────
import uuid
from datetime import datetime

@app.get("/api/geo-photos", response_model=List[schemas.GeoPhoto])
def get_geo_photos(db: Session = Depends(get_db)):
    return db.query(models.GeoPhoto).order_by(models.GeoPhoto.timestamp.desc()).all()

@app.post("/api/geo-photos", response_model=schemas.GeoPhoto)
def create_geo_photo(photo: schemas.GeoPhotoCreate, db: Session = Depends(get_db)):
    photo_id = f"photo-{uuid.uuid4().hex[:8]}"
    db_photo = models.GeoPhoto(
        id=photo_id,
        filename=photo.filename,
        image_data=photo.image_data,
        lat=photo.lat,
        lon=photo.lon,
        location_name=photo.location_name or f"Lat: {photo.lat:.4f}, Lon: {photo.lon:.4f}",
        hazard_type=photo.hazard_type or "Observation",
        description=photo.description or "",
        user_name=photo.user_name or "Field Officer",
        timestamp=photo.timestamp or datetime.now().isoformat()
    )
    db.add(db_photo)
    db.commit()
    db.refresh(db_photo)
...
@app.delete("/api/geo-photos/{photo_id}")
def delete_geo_photo(photo_id: str, db: Session = Depends(get_db)):
    db_photo = db.query(models.GeoPhoto).filter(models.GeoPhoto.id == photo_id).first()
    if not db_photo:
        raise HTTPException(status_code=404, detail="Geo Photo not found")
    db.delete(db_photo)
    db.commit()
    return {"message": f"Photo {photo_id} deleted successfully"}

@app.post("/api/vehicles/{vehicle_id}/gps", response_model=schemas.Vehicle)
def update_vehicle_gps(vehicle_id: str, gps_data: schemas.VehicleUpdate, db: Session = Depends(get_db)):
    db_vehicle = db.query(models.Vehicle).filter(models.Vehicle.id == vehicle_id).first()
    if not db_vehicle:
        raise HTTPException(status_code=404, detail="Vehicle not found")
    
    if gps_data.lat is not None:
        db_vehicle.lat = gps_data.lat
    if gps_data.lon is not None:
        db_vehicle.lon = gps_data.lon
    if gps_data.speed is not None:
        db_vehicle.speed = gps_data.speed
    if gps_data.location is not None:
        db_vehicle.location = gps_data.location
    db_vehicle.lastUpdate = datetime.now().strftime("%I:%M %p")
